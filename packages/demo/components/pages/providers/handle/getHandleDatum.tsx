...
export default function GetHandleDatum({ provider, handle, setHandle }) {
  return (
    <>
      <SectionTwoCol
        sidebarTo="getHandleDatum"
        header="getHandleDatum"
...
function LeftContent({ handle }) {
  ...
  code += `const handleDatum = await handleProvider.getHandleDatum('${handle}');`;
...
function RightContent({ provider, handle, setHandle }) {
  ...
  async function runDemo() {
    ...
    try {
      const res = await provider.getHandleDatum(handle);
      setResponse(res);
    } catch (error) {
  ...
  return (
    <Card>
      ...
      <Codeblock
        data={`const handleDatum = await handleProvider.getHandleDatum('${handle}');`}
        isJson={false}
      />
